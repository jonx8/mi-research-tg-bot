...
class WeeklyCheckInHandlers:
    ...
    async def handle_weekly_craving_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Шаг 2: ввод уровня тяги (1-10)."""
        user_input = update.message.text.strip()
        checkin_id = context.user_data.get('pending_weekly_id')
        if not checkin_id:
            await update.message.reply_text("Нет активного чек‑ина.")
            return

        checkin = await self._weekly_service.get_by_id(checkin_id)
        if not checkin or checkin.completed_at:
            await update.message.reply_text("Чек‑ин уже завершён.")
            context.user_data.pop('pending_weekly_id', None)
            return

        try:
            craving = int(user_input)
        except ValueError:
            await update.message.reply_text("⚠️ Введите целое число от 1 до 10.")
            return
